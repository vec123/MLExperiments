...
print(len(data[:,0]))
latent_samples = np.random.normal(0, 1, (len(data[:,0]), 2))* np.sqrt(pca.explained_variance_)  # White noise in 2D
...
#plt.scatter(generated_samples[:, 0], generated_samples[:, 1], color='blue', label='PCA Generated Samples', alpha=0.5)
...
plt.savefig("linear_distribution.png")
